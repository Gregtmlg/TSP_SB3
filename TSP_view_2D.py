...
class TSPView2D:
    ...
    def __locate_on_window(self, x, y):
        w_x = (x - self.map_min_x + 1.5) * self.grid_size
        w_y = (self.map_max_y - y + 1.5) * self.grid_size
        return w_x, w_y
    ...
    def __draw_grid(self):
        blockSize = self.grid_size #Set the size of the grid block
        window_size_x, window_size_y = self.__get_window_size()
        for x in range(1*blockSize, window_size_x -1*blockSize, blockSize):
            for y in range(1*blockSize, window_size_y-1*blockSize, blockSize):
                rect = pygame.Rect(x, y, blockSize, blockSize)
                pygame.draw.rect(self.game_surface, (50, 50, 50), rect, 1)
